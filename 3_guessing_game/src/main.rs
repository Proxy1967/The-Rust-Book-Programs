...
fn main() {
    ...
    // generate a random number that is local to the current thread of execution, `gen_range` is inclusive of 1 and 100
    let secret_number = rand::thread_rng().gen_range(1..=100);

    // !! USED ONLY FOR DEBUGGING, NOT IN FINAL CODE !!
    ...
    loop {
        ...
        io::stdin() // `stdin()` function reads the standard input
            .read_line(&mut guess) // `read_line` from standard input passing a mutable reference to the `guess` variable
